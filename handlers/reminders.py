...
from handlers.reminder_parsing import (
    WEEKDAY_CODES, WEEKDAY_NAMES,
    looks_like_important_reminder, looks_like_natural_reminder, now_taipei,
    parse_custom_reminder_time, parse_event_datetime_input, parse_hhmm,
    parse_recurring_rule, parse_reminder_text, parse_snooze_input, weekday_names,
)
...
def _reminder_preview_text(kind: str, who: str, dt: datetime, content: str) -> str:
    type_label = "重要提醒" if kind == "priority" else "一般提醒"
    return (
        "我理解成：\n\n"
        f"類型：{type_label}\n"
        f"對象：{who}\n"
...
def _reminder_preview_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("✅ 建立", callback_data="rp:create")],
        [
            InlineKeyboardButton("✏️ 改時間", callback_data="rp:edit_time"),
            InlineKeyboardButton("✏️ 改內容", callback_data="rp:edit_content"),
        ],
        [InlineKeyboardButton("❌ 取消", callback_data="cancel")],
    ])


async def _show_reminder_preview(update: Update, state: dict, *, edit: bool = False):
    text = _reminder_preview_text(state["kind"], state["who"], state["dt"], state["content"])
    keyboard = _reminder_preview_keyboard()
...
async def handle_reminder_preview(update: Update, ctx: ContextTypes.DEFAULT_TYPE, text: str, kind: str):
    user_id = update.effective_user.id
    who, dt, content = parse_reminder_text(text)
    if not (who and dt and content):
        await reply(update, "我還沒能完整理解這則提醒，請改用標準格式或換個說法。")
        return
    if dt <= now_taipei():
    ...
    await _show_reminder_preview(update, user_states[user_id])


async def _start_regular_reminder_from_parts(update: Update, ctx: ContextTypes.DEFAULT_TYPE,
                                             who: str, dt: datetime, content: str):
    user_id = update.effective_user.id
...
async def _start_priority_reminder_from_parts(update: Update, ctx: ContextTypes.DEFAULT_TYPE,
                                              who: str, dt: datetime, content: str):
    user_id = update.effective_user.id
    display = update.effective_user.first_name or who
    user_states[user_id] = {
...
async def cb_reminder_preview(update: Update, ctx: ContextTypes.DEFAULT_TYPE, action: str):
    q = update.callback_query
    await q.answer()
    user_id = update.effective_user.id
    state = user_states.get(user_id, {})
    if state.get("action") != "preview_reminder":
        await q.edit_message_text("這個預覽已經失效，請重新輸入提醒。")
        return

    if action == "create":
        await q.edit_message_text("✅ 已確認，正在建立提醒...")
        if state["kind"] == "priority":
            await _start_priority_reminder_from_parts(update, ctx, state["who"], state["dt"], state["content"])
        else:
            user_states.pop(user_id, None)
            await _start_regular_reminder_from_parts(update, ctx, state["who"], state["dt"], state["content"])
        return

    if action == "edit_time":
        state["action"] = "preview_edit_time"
        await q.edit_message_text(
            "⏰ 請輸入新的提醒時間：\n"
            "例如：明晚八點、明天 09:30、06/10 18:00、09:30")
        return

    if action == "edit_content":
        state["action"] = "preview_edit_content"
        await q.edit_message_text(f"✏️ 請輸入新的提醒內容：\n目前：{state['content']}")
        return

    await q.edit_message_text("❓ 未知的預覽操作。")
...
async def handle_reminder(update: Update, ctx: ContextTypes.DEFAULT_TYPE, text: str):
    ...
    await _start_regular_reminder_from_parts(update, ctx, who, dt, content)
...
async def handle_priority_reminder(update: Update, ctx: ContextTypes.DEFAULT_TYPE, text: str):
    ...
    await _start_priority_reminder_from_parts(update, ctx, who, dt, content)
...
async def _handle_reminder_state(update: Update, ctx: ContextTypes.DEFAULT_TYPE, state_store: dict, action: str, text: str):
    user_id = update.effective_user.id
    if action == "preview_edit_time":
        state = user_states.get(user_id, {})
        new_dt = parse_event_datetime_input(text, state.get("dt"))
        if not new_dt:
            await reply(update,
                "❌ 我讀不懂這個時間，請輸入例如：明晚八點、明天 09:30、06/10 18:00、09:30")
            return
        if new_dt <= now_taipei():
            await reply(update, "⚠️ 提醒時間不能設定在過去喔！")
            return
        state["dt"] = new_dt
        state["action"] = "preview_reminder"
        await _show_reminder_preview(update, state)
        return

    elif action == "preview_edit_content":
        state = user_states.get(user_id, {})
        content = text.strip()
        if not content:
            await reply(update, "❌ 內容不能是空的，請再輸入一次。")
            return
